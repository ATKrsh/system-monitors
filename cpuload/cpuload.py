...
class CPULoadWorker(QThread):
    load_updated = pyqtSignal(float, str, float)  # load, top_proc_name, top_proc_pct
    ...
    def run(self):
        ...
        while self._running:
            ...
            self.load_updated.emit(load, self._top_proc_name, self._top_proc_pct)
...
class CPULoadWidget(QWidget):
    def __init__(self):
        ...
        self.alert_wav_path = create_temp_wav("alert", 950, 120)
        ...
        self.setWindowTitle("CPU Load Monitor")
    ...
    def _setup_tray(self):
        ...
        self.tray.setToolTip("CPU Load Monitor")
        ...
        self.act_sound = QAction("High Load Alarm On", self, checkable=True)
    ...
    def _on_load_updated(self, val, top_name="System", top_pct=0.0):
        ...
        if self.sound_enabled and val >= 92.0:
            self._trigger_load_sound()

    def _trigger_load_sound(self):
        now = time.time()
    ...
    def _update_ui_state(self):
        ...
        if self.transition_mode == "smooth":
            diff = self.raw_load - self.displayed_load
            self.displayed_load += diff * 0.15
        else:
            self.displayed_load = self.raw_load

        c = self._get_color_for_percentage(self.displayed_load, self.transition_mode)
    ...
    def paintEvent(self, event):
        ...
        core_color = self._get_color_for_percentage(self.displayed_load, self.transition_mode)
        ...
        if self.display_mode == 3:
            self.draw_animated_icon(p, cx, cy, self.displayed_load, core_color)
            return
        ...
        # 3. Draw Programmatic Animated CPU/Microchip Icon in center
        p.setBrush(Qt.NoBrush)
        icon_color = QColor(255, 255, 255, 210)
        p.setPen(QPen(icon_color, 1.1, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))

        # Draw chip body (12x12 rounded square)
        p.drawRoundedRect(QRectF(cx - 5.5, cy - 5.5, 11.0, 11.0), 1.5, 1.5)

        # Draw pins (lines sticking out)
        # Left pins
        p.drawLine(QPointF(cx - 7.5, cy - 2.5), QPointF(cx - 5.5, cy - 2.5))
        p.drawLine(QPointF(cx - 7.5, cy + 2.5), QPointF(cx - 5.5, cy + 2.5))
        # Right pins
        ...
        # Animated pulsing core (grows and fades based on load)
        pulse_speed = 2.0 + (self.displayed_load / 100.0) * 8.0
        pulse = 0.5 + 0.5 * math.sin(time.time() * pulse_speed)
        
        p.setPen(Qt.NoPen)
        # Core color glows white with opacity pulsing
        p.setBrush(QBrush(QColor(255, 255, 255, int(150 + 105 * pulse))))
        core_sz = 3.5 + 1.2 * pulse
        p.drawRect(QRectF(cx - core_sz/2.0, cy - core_sz/2.0, core_sz, core_sz))
    ...
    def _restore_position(self, config):
        ...
        default_y = screen.height() - self.height() - 170  # stack vertically
