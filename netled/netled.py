...
class NETLEDWidget(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Net LED Monitor")
    ...
    def _setup_tray(self):
        ...
        self.tray.setToolTip("Net LED Monitor")
    ...
    def _on_net_activity(self, has_download, has_upload):
        ...
        if self.sound_enabled:
            self._play_activity_sound(has_download, has_upload)

    def _play_activity_sound(self, has_download, has_upload):
        now = time.time()
    ...
    def _update_ui_state(self):
        ...
        has_active_glow = self.is_downloading or self.is_uploading
        if has_active_glow:
    ...
    def paintEvent(self, event):
        ...
        if self.is_downloading or self.is_uploading:
            ...
            if self.is_downloading:
                glow_color = QColor(255, 230, 0, 140) # Tight Yellow (Download)
            else:
                glow_color = QColor(180, 0, 255, 140) # Tight Purple (Upload)
        ...
        if self.is_downloading:
            core_color = QColor(255, 210, 0) # Active Yellow
        elif self.is_uploading:
            core_color = QColor(150, 20, 230) # Active Purple
        else:
            core_color = QColor(100, 100, 100) # Idle Grey

        pct = 100.0 if (self.is_downloading or self.is_uploading) else 0.0
        ...
        if self.display_mode == 2:
            arrow_color = QColor(255, 255, 255, 240) if (self.is_downloading or self.is_uploading) else QColor(160, 160, 160, 140)
            ...
            if self.is_downloading:
                ...
                # Left Up Arrow (Upload style placeholder)
                ...
                # Right Down Arrow (Download style placeholder)
                p.drawLine(QPointF(cx + 2.5, cy - 3.5), QPointF(cx + 2.5, cy + 3.5))
                p.drawLine(QPointF(cx + 2.5, cy + 3.5), QPointF(cx + 1.0, cy + 1.5))
                p.drawLine(QPointF(cx + 2.5, cy + 3.5), QPointF(cx + 4.0, cy + 1.5))
    ...
    def change_volume(self, val):
        self.volume = val
        self.sound_down.setVolume(val / 100.0)
        self.sound_up.setVolume(val / 100.0)
    ...
    def _restore_position(self, config):
        ...
        default_x = screen.width() - self.width() - 120
